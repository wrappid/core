...
export default function LayoutManager(props) {
  ...
  React.useEffect(() => {
    let developmentData = { ...development };

    if (layoutName && componentRegistry && Object.keys(componentRegistry).includes(layoutName) && componentRegistry[layoutName]?.comp) {
      developmentData.layoutNotFound = false;
      developmentData.layout = layoutName;
      developmentData.renderedLayout = layoutName;
    } else {
      developmentData.layoutNotFound = true;
      developmentData.renderedLayout = BlankLayout.name;
    }
    
    if (pageName && componentRegistry && Object.keys(componentRegistry).includes(pageName) && componentRegistry[pageName]?.comp) {
      developmentData.pageNotFound = false;
      developmentData.page = pageName;
      developmentData.renderedPage = pageName;
    } else {
      developmentData.pageNotFound = true;
      developmentData.renderedPage = ComponentNotFound.name;
    }

    dispatch({ payload: developmentData, type: UPDATE_DEVELOPMENT_DATA });
  ...
  React.useEffect(() => {
    ...
    //   dispatch({ payload: { layoutMismatch: true, renderedLayout: LayoutMismatch.name }, type: UPDATE_DEVELOPMENTData_DATA });
    // }
  ...
  const renderLayoutEmbeddedPage = () => {
     ...
     */
    if (checkIfLayoutMismatch(LayoutComponent, PageComponent)) {
